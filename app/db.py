...
def upsert_file(meta, content="", status="metadata_only", error=None):
    with connect() as conn:
        ...
        INSERT INTO files(
            ...
            modified_at,size,content,indexed_at,status,error
        )
        VALUES(
            :item_id,:name,:path,:web_url,:mime_type,:extension,
            :modified_at,:size,:content,CURRENT_TIMESTAMP,:status,:error
        )
        ...
        """, {**meta, "content": content or "", "status": status, "error": error})
